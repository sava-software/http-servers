...
final class FusionAuthConformanceTest {
  ...
  ///
  /// Documented divergence. RFC 9112 s3.2 makes Host mandatory only from HTTP/1.1 onwards,
  /// and the jdk and jetty backends serve the Host-less 1.0 form, but java-http requires a
  /// non-blank Host on every request and answers 400 without one (oracle: java-http itself,
  /// probed on the wire 2026-09-12 at the version resolved here - origin-form and
  /// absolute-form targets, HTTP/1.0 and HTTP/1.1, all refused alike when Host is missing or
  /// blank). Both halves are pinned: the refusal, and the 200 the cross-backend contract
  /// promises once Host is supplied.
  @Test
  void http10RequestIsAnswered() throws Exception {
    ...
    final var hostless = rawExchange(port, "GET /p HTTP/1.0\r\n\r\n");
    assertEquals(400, rawStatus(hostless.response()), hostless.response());
  ...
  void noContentAndNotModifiedCrossTheWireWithoutABody() throws Exception {
    ...
    // and on the raw wire: java-http sends "content-length: 0" on both bodyless statuses
    // (its own header case, which rawHeader matches case-insensitively), and chunks neither
