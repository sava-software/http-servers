...
The population dropped from 71 mutants to 61 on 2026-07-22 when the
`handlePreFlight` handler seam was removed: `JettyController` now writes the
...
- **Handled-flag family** (`JettyController` 66/78/81/87,
  `JettyQueryHandler.handle` 42, `JettyCachedJsonResponseHandler.handle` 27):
...
- **Wildcard-bind family** (`JettyServerBuilder.initRestServer` 33 both
  skip-directions, 34 `setHost` removal): not setting the connector host
...
- `JettyController` 35 (`EQUAL_IF` on the blank-ACRM check): treating a blank
...
- `JettyController` 69 (`EQUAL_IF` on `origin != null`): forcing the branch
...
- `JettyController` 85 (`setStatus(500)` removal in the catch):
  `callback.failed(throwable)` on the next line produces the same 500.
- `JettyServerBuilder` 23 (`setVirtualThreadsExecutor` removal): thread
  placement only — wire-invisible, and asserting it would couple the test
  to Jetty thread-pool internals.
- `JettyServerBuilder` 28 (`setSendXPoweredBy(false)` removal): the flag's
